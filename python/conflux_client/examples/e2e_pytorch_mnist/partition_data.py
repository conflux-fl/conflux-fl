#!/usr/bin/env python3
"""Downloads real MNIST (torchvision) and partitions a subsample of it
across N federated clients — see the E2E harnesses guide ("Option B").

Subsampled (not the full 60k/10k split) to keep the demo's wall-clock
time reasonable with a handful of local SGD steps per round; increase
--n-train/--n-held-out for a more realistic (slower) run.
...
def main() -> None:
    ...
    parser.add_argument("--n-train", type=int, default=3000, help="subsample size for the federated pool")
    ...
    parser.add_argument("--data-root", default="/tmp/conflux_mnist")
    ...
    transform = torchvision.transforms.Compose(
        [
            ...
            torchvision.transforms.Normalize((0.1307,), (0.3081,)),  # standard MNIST stats
        ]
    )
    train_ds = torchvision.datasets.MNIST(root=args.data_root, train=True, download=True, transform=transform)
    test_ds = torchvision.datasets.MNIST(root=args.data_root, train=False, download=True, transform=transform)
