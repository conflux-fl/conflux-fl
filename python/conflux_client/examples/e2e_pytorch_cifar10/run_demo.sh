#!/usr/bin/env bash
# End-to-end demo: real CIFAR-10 + a real PyTorch CNN, federated across
# N clients through Conflux FL. Same structure as the MNIST demo's
# run_demo.sh — see that file's comments for what each step does; only
# the model/dataset differ here.
#
# Usage:
...
#
# First run downloads CIFAR-10 (~170MB) to /tmp/conflux_cifar10 —
# cached after. Run from this directory with the venv already active
# (see README.md).
set -euo pipefail
...
echo "model dimension (flattened CNN parameter count): $DIM"

echo ""
echo "=== 2. downloading + partitioning CIFAR-10 (N=$N_CLIENTS clients, split=${SPLIT_FLAGS[1]}) ==="
...
CONFLUX_ROUND_TIMEOUT_SECS=120 \
...
python3 "$SCRIPT_DIR/eval_client.py" \
  ...
  --rounds "$ROUNDS" --timeout 420 | tee "$WORK_DIR/eval.log"
