#!/usr/bin/env bash
# End-to-end demo: the real Shakespeare corpus + a real character-level
# GRU, federated across N clients through Conflux. Same structure as the
# MNIST demo's run_demo.sh — see that file's comments for what each step
# does; only the model and the partitioning differ.
#
# What this harness adds over the image ones: a *sequence* task with a
# *recurrent* model, and a federated partition that is non-IID because
# each client is a different speaking role — not because a Dirichlet
# knob was turned. See partition_data.py's own docstring.
#
# Usage:
#   ./run_demo.sh [AGGREGATOR] [N_CLIENTS] [ROUNDS]
#   ./run_demo.sh krum 5 15 --poison --no-reputation
#   ./run_demo.sh fedavg 5 15 --dirichlet                          # by-role (non-IID)
#
# `--dirichlet` here selects the by-role partition rather than a
# Dirichlet label skew: a character-level language model has no class
# labels to skew, so the natural per-speaker split is what "non-IID"
# means for this dataset. The flag keeps the same name so benchmark.py's
# `--splits` sweep works unchanged across every harness.
#
# First run downloads the corpus (~1MB) to /tmp/conflux_shakespeare — cached after.
...
# `--dirichlet` selects the natural by-role partition (this dataset's
# reason for existing here); its absence gives the IID control.
SPLIT_FLAGS=(--split iid)
if [ "$DIRICHLET" = true ]; then SPLIT_FLAGS=(--split role); fi

# Tuned for the GRU: the MLP harness's lr=0.1 makes a recurrent net
# oscillate rather than converge, even with the gradient clipping in
# model.py's train_steps.
LR=0.5
STEPS=20
...
echo ""
echo "=== 2. downloading + partitioning Shakespeare (N=$N_CLIENTS clients, split=${SPLIT_FLAGS[1]}) ==="
