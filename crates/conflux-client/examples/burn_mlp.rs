...
fn parse_args() -> Args {
    ...
    while i < argv.len() {
        ...
        match argv[i].as_str() {
            "--help" | "-h" => {
                println!(
                    "burn_mlp — a real Burn MLP ClientApp driving the catalog aggregators in-process\n\n\
                     USAGE: cargo run --example burn_mlp -p conflux-client --features burn -- [FLAGS]\n\n\
                     FLAGS (with defaults):\n\
                     \x20 --aggregator <name>   catalog method to aggregate with   [fedavg]\n\
                     \x20 --clients <n>         honest clients in the federation   [5]\n\
                     \x20 --attackers <n>       additional poisoned clients        [0]\n\
                     \x20 --rounds <n>          federated rounds                   [8]\n\
                     \x20 --steps <n>           local SGD steps per client per round [40]\n\n\
                     Prints `held_out_accuracy=<value>` every round; the last line is what\n\
                     `conflux-baselines` reads."
                );
                std::process::exit(0);
            }
