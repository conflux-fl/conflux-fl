...
/// How much of a failed harness's stdout and stderr to show. Enough for
/// a Python traceback or a cargo error; not the whole build log.
...
fn harness_result(what: &str, output: &std::process::Output) -> Option<f64> {
    ...
    // The harness scripts explain their own failures on *stdout* (a busy
    // port, a missing venv, a health gate that did not pass), while
    // stderr is often just download progress — so show both tails.
    ...
    for (name, text) in [("stdout", &stdout), ("stderr", &stderr)] {
        let tail: Vec<&str> = text
            ...
            .collect();
        if !tail.is_empty() {
            eprintln!("  last {} line(s) of its {name}:", tail.len());
