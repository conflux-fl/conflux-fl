...
out-of-tree research — now build on these crates, and "whatever compiles
today" is not something either can plan against.

## The promise, in one line

**At `0.x`, breaking changes land in minor versions and are listed in
[CHANGELOG.md](../CHANGELOG.md).** That is a real commitment to
*disclosure*, not to stability. Nothing here is `1.0`, and the `0.` is load-bearing.

## Why not 1.0 yet

Three things would have to be true first, and none is:

1. **The public surface has not settled.** The run-up to `0.1.0` alone
...
| Anything named in a phase brief as provisional | The brief says so, and the brief is the record. |
...
| **Backends** | `Registry`, `Store`, `NodeAllowlist`, `PrivacyRoundLog` | Stable in shape, growing in surface. `PrivacyRoundLog` gained per-client methods in without breaking existing implementors. |
...
A breaking change gets a changelog entry stating what broke, why, and
