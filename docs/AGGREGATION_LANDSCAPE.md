...
|---|---|---|
| **FedNova** | Wang, Wang, Nusrat, Poor & Rajan, 2020 | Per-client local-step-count-normalized weighting. Fits `AveragingWeighting` cleanly (a new member, `StepNormalizedWeighting`) — **but** needs a new field on `ClientDelta`/`DeltaChunk` (local step count) alongside the existing `num_samples`, so it's a `conflux-proto` schema change, not just a Rust trait impl. |
...
| **SCAFFOLD** | Karimireddy, Kale, Mohri, Reddi, Stich & Suresh, 2020 | Needs each client to also send a **control-variate delta**, same dimensionality as the model delta, combined into the correction on both client and server. Either doubles the payload by convention (client packs `[weight_delta; control_variate_delta]`) or needs a real `conflux-proto` extension — bigger than FedNova's one scalar field. |
