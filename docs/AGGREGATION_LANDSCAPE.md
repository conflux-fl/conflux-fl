...
| **Zeno / Zeno++** ✅ *Zeno built 2026-09-02* | Xie, Koyejo & Gupta, 2019 / 2020 | Server scores each update by whether it improves loss on a held-out server-side validation set, keeps the top-scoring subset. Also anchored to an independent, server-controlled signal. Shipped as `zeno` (trusted family): suspicion score = sidecar's held-out improvement − `ρ‖update‖²`, drop the `b` lowest, unweighted mean of the rest — consuming the sidecar's `ScoreUpdates` RPC that had been serving no one since ADR 0011 landed. **Zeno++ (2020) is declined, with a reason rather than a shrug**: it is *fully asynchronous* SGD — the server scores and applies each gradient the moment it arrives, against whatever the model currently is; there is no batch and no round. That is an execution model, not an aggregation rule: it cannot be expressed through `Aggregator::aggregate(batch) -> weights` at all, and Conflux's spec-§8 pipeline (buffer → flush on quorum/timeout → aggregate) is round-based by design. A "batched Zeno++" would be a variant the paper never defined, which ADR 0008 exists to forbid shipping under the paper's name. If Conflux ever grows an asynchronous pipeline mode, Zeno++ is the first method it should carry — until then, round-based Zeno (2019, shipped) covers the trusted-scoring capability. |
